...
export default function PromoFloatingBanner() {
  ...
  useEffect(() => {
    // Solo mostrar si no ha sido descartado en esta sesión
    if (sessionStorage.getItem('promo_dismissed')) {
      setIsDismissed(true);
      return;
    }

    // Retrasar la aparición para no abrumar al usuario al entrar
      ...
      {isVisible && (
        <motion.div
          ...
          className={`fixed bottom-4 md:bottom-6 left-4 md:left-6 z-[100] w-[calc(100%-2rem)] max-w-[340px] rounded-2xl border shadow-2xl overflow-hidden backdrop-blur-xl
            ${isDark 
              ? 'bg-slate-950/95 border-blue-500/30 shadow-blue-900/40 text-white' 
              : 'bg-white/95 border-blue-200 shadow-blue-900/10 text-slate-900'}`}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-2.5 px-4 flex justify-between items-center shadow-md z-10 relative">
          ...
          <div className="p-4 space-y-4 relative">
            ...
            <div className={`rounded-xl p-3 border space-y-3 relative overflow-hidden
            ...
            `}>
              <div className={`flex justify-between items-center border-b pb-2 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                <span className={`text-xs font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Precio Especial</span>
                <span className={`text-base font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>S/ 1,500</span>
              </div>
              
              <div className="relative z-10">
                <div className="absolute -left-5 -top-3">
                   <span className="flex items-center justify-center w-8 h-8 bg-amber-500 text-white text-[10px] font-black rounded-full rotate-12 shadow-lg shadow-amber-500/30">
                     -50%
                   </span>
                </div>
                <div className="flex flex-col items-end">
                  <div className="flex items-center gap-1.5 text-amber-500 mb-1 bg-amber-500/10 px-2 py-0.5 rounded text-right">
                  ...
                  <div className="flex items-baseline gap-1.5 mt-1">
                    <span className={`text-[10px] font-medium uppercase tracking-wide ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Pagas solo:</span>
                    <span className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>S/ 750</span>
                  </div>
                </div>
              </div>
            </div>
